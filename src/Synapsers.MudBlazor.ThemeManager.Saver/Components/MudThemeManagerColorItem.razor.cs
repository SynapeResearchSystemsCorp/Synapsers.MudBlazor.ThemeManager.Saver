...
public partial class MudThemeManagerColorItem : ComponentBase
{
    ...
    [Parameter]
    public EventCallback<string> ColorChanged { get; set; }    protected override void OnInitialized()
    {
    ...
    }    protected override void OnParametersSet()
    {
        ...
        // Reset isOpen state when parameters change to avoid stale popover state
        _isOpen = false;
        
        // Ensure we have valid state when component is re-initialized after navigation
        if (ThemeManager != null && string.IsNullOrEmpty(Color))
        {
            // Try to get the color from ThemeManager's palette
            string? paletteColor = ThemeManager.GetPaletteColorValue(ColorType);
            if (!string.IsNullOrEmpty(paletteColor))
            {
                Color = paletteColor;
            }
            else if (!string.IsNullOrEmpty(ThemeColor))
            {
                // Fallback to ThemeColor if palette color not found
                Color = ThemeColor;
            }
        }
